...
public class UserInterfaceImpl extends JFrame implements UserInterface {
  ...
  static String intInBaseToPaddedString(int n, int padding, int base) {
    return format("%" + padding + "s", Integer.toString(n, base))
      .replace(' ', '0');
  }
  ...
  private class TimerPanel extends JPanel {
    ...
    private String generateTime(long millies, boolean warn) {
      if (warn) return format(
        "Remaining Time: %.2f",
        (double) millies / 1000.0f
      ); else return format("Remaining Time: %d", millies / 1000L);
    }

    private TimerPanel() {
      timerField =
        new JLabel(config.turnTimeoutMillis < 0 ? "PLAY" : "GET READY...");
  ...
  private class GamePanel extends JLayeredPane {
    ...
    private Image loadImageResource(String filename) {
      ...
      if (imageResource == null) throw new RuntimeException(
    ...
    private GamePanel() {
      setPreferredSize(
        new Dimension(
          config.columns * config.cellWidth,
          config.rows * config.cellHeight
        )
      );
      ...
      for (int i = 0; i < config.deckSize; ++i) deck[i] =
        loadImageResource(
          "cards/" +
          intInBaseToPaddedString(i, config.featureCount, config.featureSize) +
          ".png"
        );
      ...
      for (int row = 0; row < config.rows; row++) {
        for (int column = 0; column < config.columns; column++) {
          ...
          tokenText[row][column].setBorder(
              BorderFactory.createLineBorder(Color.black)
            );
          tokenText[row][column].setBounds(
    ...
    private void placeToken(int player, int slot) {
      logger.log(
        Level.SEVERE,
        "player " + player + " placing token on slot " + slot
      );
      int row = slot / config.columns;
      int column = slot % config.columns;
    ...
    private void removeTokens() {
      ...
      for (int i = 0; i < config.tableSize; i++) removeTokens(i);
    }
    ...
    private void removeToken(int player, int slot) {
      logger.log(
        Level.SEVERE,
        "removing player " + player + " token from slot " + slot
      );
      int row = slot / config.columns;
      int column = slot % config.columns;
      playerTokens[player][row][column] = false;
    ...
    private String generatePlayersTokenText(int row, int column) {
      ...
      for (int player = 0; player < config.players; player++) {
        if (playerTokens[player][row][column]) text =
          text.concat(config.playerNames[player] + ", ");
      }
    ...
    public void paintComponent(Graphics g) {
      // draw card images
      for (int row = 0; row < config.rows; row++) for (
        int column = 0;
  ...
  private class PlayersPanel extends JPanel {
    ...
    private PlayersPanel() {
      ...
      this.setPreferredSize(
          new Dimension(
            config.players * config.playerCellWidth,
            config.rows * config.playerCellHeight
          )
        );
      this.playersTable = new JLabel[2][config.players];
      for (int i = 0; i < config.players; i++) {
        this.playersTable[0][i] = new JLabel(config.playerNames[i]);
        this.playersTable[0][i].setFont(
            new Font("Serif", Font.BOLD, config.fontSize)
          );
      ...
      for (int i = 0; i < config.players; i++) {
        ...
        this.playersTable[1][i].setFont(
            new Font("Serif", Font.PLAIN, config.fontSize)
          );
    ...
    private void setScore(int player, int score) {
      logger.log(
        Level.SEVERE,
        "setting player " + player + " score to " + score
      );
    ...
    private void setFreeze(int player, long millies) {
      logger.log(
        Level.SEVERE,
        "setting player " + player + " freeze to " + millies
      );
      if (millies > 0) {
        this.playersTable[0][player].setText(
            config.playerNames[player] + " (" + millies / 1000 + ")"
          );
  ...
  private class WinnerPanel extends JPanel {
    ...
    public WinnerPanel() {
      ...
      this.winnerAnnouncement.setFont(
          new Font("Serif", Font.BOLD, config.fontSize)
        );
    ...
    private void announceWinner(int[] players) {
      logger.log(
        Level.SEVERE,
        "announcing winners: " + Arrays.toString(players)
      );
      if (players.length == 1) winnerAnnouncement.setText(
        "THE WINNER IS: " + config.playerNames[players[0]] + "!!!"
      ); else {
        String text = "";
        for (int player : players) text =
          text.concat(config.playerNames[player] + " AND ");
