...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      if (config.humanPlayers > 0) logger.severe(
        "warning: running with human players with no user interface"
      );
    }
    ...
    for (int i = 0; i < players.length; i++) players[i] =
      new Player(env, dealer, table, i, i < env.config.humanPlayers);
    ...
    try {
      ...
      if (!xButtonPressed && config.endGamePauseMillies > 0) Thread.sleep(
        config.endGamePauseMillies
      );
  ...
  private static Logger initLogger() {
    ...
    try {
      ...
      handler =
        new FileHandler(
          "./logs/" + format.format(Calendar.getInstance().getTime()) + ".log"
        );
    } catch (IOException e) {
  ...
  public static void setLoggerLevelAndFormat(
    Logger logger,
    Level level,
    String format
  ) {
    Handler[] handlers = logger.getHandlers();
    if (handlers != null) Arrays
      .stream(handlers)
      .forEach(h ->
        h.setFormatter(
          new SimpleFormatter() {
