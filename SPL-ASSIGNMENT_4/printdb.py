...
def printActivities():
    print("Activities")
    acts = repo.execute_command("""SELECT * FROM activities ORDER BY date ASC""")
    for row in acts:
...
def printBranches():
    print("Branches")
    branches = repo.execute_command("""SELECT * FROM branches ORDER BY id ASC""")
    for row in branches:
        tmp =(str)(row).replace("b'", "")
        tmp = tmp.replace("'","")
        print(tmp)
...
def printActivityReport():
    ...
    acts = repo.execute_command("""SELECT a.date AS date, p.description, a.quantity FROM activities AS a JOIN products AS p ON a.product_id=p.id ORDER BY date ASC""")
    for row in acts:
        tmp =(str)(row).replace("b'", "")
        tmp = tmp.replace("'","")
        print(tmp)
